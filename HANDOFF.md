...
2. **`auto_confirmed` 状态没有任何代码会写**
   老师标记完课后家长一直不点确认，订单就永远卡在 `teacher_done`，结算走不下去。需要一个定时任务：超过 N 天自动置为 `auto_confirmed`。

### 中

3. **教师端「我的课时」和后台课时管理没有分页**
   订单多了会一次性全拉出来。

4. **`/teacher/rules` 只在前端用 localStorage 判断登录**
...
7. 家长预约后没有任何通知给教务（正在做：企业微信群机器人 webhook 推送）
8. 老师端资料填写没有完成度提示，很多老师注册完不知道要补资料
9. 后台没有数据看板（本月新增预约数、成交数、流水）
